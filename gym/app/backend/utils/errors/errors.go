package errors

import "errors"

var (
	ERR_EMAIL_IS_EMPTY                      = errors.New("Email is empty")
	ERR_LOGIN                               = errors.New("Failed to signing")
	ERR_REGISTER                            = errors.New("Failed to register")
	ERR_UPDATE_USER                         = errors.New("Failed to update user")
	ERR_UPDATE_ROLE                         = errors.New("Failed to update role")
	ERR_UPDATE_TRANSACTION                  = errors.New("Failed to update Transaction")
	ERR_UPDATE_MEMBERSHIP_PLAN              = errors.New("Failed to update membership plan")
	ERR_DELETE_USER                         = errors.New("Failed to delete user")
	ERR_DELETE_MEMBERSHIP_PLAN              = errors.New("Failed to delete membership plan")
	ERR_DELETE_ROLE                         = errors.New("Failed to delete Role")
	ERR_DELETE_TRANSACTION                  = errors.New("Failed to delete Transaction")
	ERR_CREATE_ROLE                         = errors.New("Failed to create new Role")
	ERR_CREATE_ABSENSI                      = errors.New("Failed to create Absensi")
	ERR_CREATE_MEMBERSHIP_PLAN              = errors.New("Failed to create new Membership Plan")
	ERR_CREATE_TRANSACTION                  = errors.New("Failed to create new Transaction")
	ERR_CREATE_TRANSACTION_DETAIL           = errors.New("Failed to create new Transaction Detail")
	ERR_SAVE_TRANSACTION_MEMBER_DETAIL      = errors.New("Failed to save Transaction Member Detail")
	ERR_PASSWORD_IS_EMPTY                   = errors.New("Password is empty")
	ERR_PHONE_NUMBER_IS_EMPTY               = errors.New("Phone number is empty")
	ERR_ROLE_IS_EMPTY                       = errors.New("Role is empty")
	ERR_BCRYPT_PASSWORD                     = errors.New("Failed to bcrypt password")
	ERR_OLD_PASSWORD_IS_EMPTY               = errors.New("You need to input old password to change password")
	ERR_TOKEN                               = errors.New("Failed to create new token")
	ERR_GET_DATA                            = errors.New("Failed to get data from database")
	ERR_NOT_FOUND                           = errors.New("Data not Found")
	ERR_TRANSACTION_DETAIL_NOT_FOUND        = errors.New("Transaction Detail not Found")
	ERR_TRANSACTION_MEMBER_DETAIL_NOT_FOUND = errors.New("Transaction Member Detail not Found")
	ERR_MEMBERSHIP_PLAN_NOT_FOUND           = errors.New("Membership Plan not Found")
	ERR_USER_NOT_FOUND                      = errors.New("User not Found")
	ERR_TRANSACTION_MEMBER_EMPTY            = errors.New("The member data is missing in transaction detail")
	ERR_EMAIL_NOT_FOUND                     = errors.New("Email not found")
	ERR_WRONG_PASSWORD                      = errors.New("Wrong password")
	ERR_SAME_PASSWORD                       = errors.New("It's the same password like the previous one")
	ERR_QTY_EMPTY                           = errors.New("Quantity is empty")
	ERR_TRANSACTION_DETAIL_EMPTY            = errors.New("Transaction Detail is empty")
	ERR_MEMBERSHIP_NAME_EMPTY               = errors.New("Membership plan is empty")
	ERR_MEMBERSHIP_DURATION_EMPTY           = errors.New("Membership duration is empty")
	ERR_MEMBERSHIP_PRICE_EMPTY              = errors.New("Membership price is empty")
	ERR_UNAUTHORIZE                         = errors.New("Unauthorize user")
	ERR_COMPLETED_TRANSACTION               = errors.New("Completed transaction can't be change")
)
